...
export default function VoiceEnrollment({
  ...
  const startEnroll = async () => {
    ...
    if (enrolledVoices.length >= MAX_ENROLLMENTS) {
      alert('Maksimal 3 voice enrollment. Hapus salah satu untuk menambah baru.');
      return;
    }
  ...
  const handleRenameVoice = async (voiceId: string, newLabel: string) => {
    ...
    try {
      ...
      setEnrolledVoices(prev => 
        prev.map(v => v.id === voiceId ? { ...v, label: newLabel.trim() } : v)
      );
      setEditingId(null);
      setEditingLabel('');
    } catch (err) {
            ...
            <div className="space-y-1">
              {enrolledVoices.map((voice) => (
                <VoiceItem
                  ...
                  inputRef={editingId === voice.id ? editInputRef : null}
                />
              ))}
            </div>
          ) : (
